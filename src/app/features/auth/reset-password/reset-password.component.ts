...
export class ResetPasswordComponent {
  ...
  constructor(
    ...
    // Guard: must come from a verified OTP session
    ...
    try {
      ...
      if (!data.verified || data.email !== this.email || Date.now() > data.expires) {
  ...
  onSubmit() {
    ...
    const success = this.auth.resetPassword(this.email, this.password);

    if (success) {
      sessionStorage.removeItem('alh_otp_data');
      this.successMsg = 'Password reset successfully! Signing you in...';
